...
import com.sydh.iot.tsdb.config.InfluxConfig;
...
public class InfluxLogService implements ILogService {
    ...
    public int deleteDeviceLogByDeviceNumber(String deviceNumber) {
        ...
        String fluxQuery = String.format("from(bucket: \"%s\")\n" +
                ...
                "  |> count()", influxConfig.getBucket(), deviceNumber);

        // 执行查询以获取要删除的记录数
        long count = queryApi.queryRaw(fluxQuery, influxConfig.getOrg()).length();

        // 构建删除语句
        String deleteQuery = String.format("delete from \"%s\" where \"serialNumber\" == \"%s\"", influxConfig.getBucket(), deviceNumber);
